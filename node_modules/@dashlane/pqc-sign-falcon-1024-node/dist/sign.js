...
async function signBuilder(useFallback = false, wasmFilePath = undefined) {
    let Module = {};
    if (wasmFilePath) {
        Module.locateFile = () => {
            return wasmFilePath;
    ...
    if (useFallback) {
        Module = (await createJSSIGNNativeCaller.default(Module));
    }
    else {
        try {
            Module = (await createWASMSIGNNativeCaller.default(Module));
        }
        catch (err) {
    ...
    let publicKeyBytes, privateKeyBytes, signatureBytes;
    ...
    return {
        ...
        keypair: async () => {
            ...
            return initiated.then(async () => {
                ...
                try {
                    const returnValue = await Module.ccall('sign_keypair', 'number', ['number', 'number'], [publicKeyBuffer, privateKeyBuffer], { async: true });
        ...
        sign: async (message, privateKey) => {
            ...
            return initiated.then(async () => {
                ...
                try {
                    const returnValue = await Module.ccall('sign_signature', 'number', ['number', 'number', 'number', 'number', 'number'], [signatureBuffer, signatureLengthBuffer, messageBuffer, message.byteLength, privateKeyBuffer], { async: true });
        ...
        verify: async (signature, message, publicKey) => {
            ...
            return initiated.then(async () => {
                ...
                try {
                    const returnValue = await Module.ccall('sign_verify', 'number', ['number', 'number', 'number', 'number', 'number'], [signatureBuffer, signature.byteLength, messageBuffer, message.byteLength, publicKeyBuffer], { async: true });
